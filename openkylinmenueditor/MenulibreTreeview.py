...
class Treeview(Gtk.Box):
    ...
    def __init__(self, parent):
        ...
        self._launchers_only_saved_model = None
        ...
        sep.set_draw(True)
        self._toolbar.add(sep)

        img = Gtk.Image.new_from_icon_name(
            'view-list-symbolic', Gtk.IconSize.MENU)
        self._launchers_only_button = Gtk.ToggleToolButton.new()
        self._launchers_only_button.set_icon_widget(img)
        self._launchers_only_button.set_tooltip_text(_("Show Launchers Only"))
        self._launchers_only_button.connect(
            'toggled', self._on_launchers_only_toggled)
        self._toolbar.add(self._launchers_only_button)
    ...
    def _on_launchers_only_toggled(self, widget):
        """Toggle between showing all items and showing only launchers."""
    ...
    def _enable_launchers_only(self):
        """Switch the treeview to a flat list of launchers only."""
        model = self._treeview.get_model()
        if model is None:
            return

        # Unwrap filter model if search is active
        if isinstance(model, Gtk.TreeModelFilter):
            model = model.get_model()

        self._launchers_only_saved_model = model

        # Build a flat ListStore with the same column types as the TreeStore
        col_types = [model.get_column_type(i)
                     for i in range(model.get_n_columns())]
        list_store = Gtk.ListStore(*col_types)

        self._collect_launchers(model, None, list_store)

        self._treeview.set_model(list_store)
        self._treeview.set_show_expanders(False)
        self._lock_menus = True

        # Disable move/sort buttons (editing position is meaningless in this view)
        self._move_up_button.set_sensitive(False)
        self._move_down_button.set_sensitive(False)
        self._sort_button.set_sensitive(False)

        # Select the first launcher if nothing else is selected
        first = list_store.get_iter_first()
        if first is not None:
            self._treeview.set_cursor(list_store.get_path(first))

    def _disable_launchers_only(self):
        """Restore the full tree after launchers-only mode."""
        if self._launchers_only_saved_model is None:
            return

        # Remember which launcher was selected so we can restore selection
        row_data = None
        f_model, f_iter = self._get_selected_iter()
        if f_model is not None and f_iter is not None:
            row_data = f_model[f_iter][:]

        model = self._launchers_only_saved_model
        self._launchers_only_saved_model = None
        ...
        self._sort_button.set_sensitive(True)

    def _collect_launchers(self, model, parent, list_store):
        """Recursively collect APPLICATION/LINK items into a flat ListStore."""
        for i in range(model.iter_n_children(parent)):
            treeiter = model.iter_nth_child(parent, i)
            if treeiter is None:
                continue
            ...
            if item_type in (MenuItemTypes.APPLICATION,  # type: ignore
                ...
                list_store.append(model[treeiter][:])
            elif item_type == MenuItemTypes.DIRECTORY:  # type: ignore
                self._collect_launchers(model, treeiter, list_store)
...
# Search
    ...
    def set_searchable(self, searchable, expand=False):
        ...
        if searchable:
            # Deactivate launchers-only mode before entering search mode
            if self._launchers_only_saved_model is not None:
                self._launchers_only_button.set_active(False)
    ...
    def _treeview_match(self, model, treeiter, query):
        ...
        desktop = desktop.replace("openkylinmenueditor-", "")
