...
def getDirectoryNameFromCategory(name):  # noqa
    ...
    directories after category addition without having to restart openkylinmenueditor."""
...
def getSaveFilename(name, filename, item_type, force_update=False):  # noqa
    ...
    if unique or not os.access(filename, os.W_OK):
        ...
        if unique:
            basename = "openkylinmenueditor-" + name.lower().replace(' ', '-')
...
def determine_bad_desktop_files():
    ...
    try:
        result = subprocess.run(['openkylinmenueditor-menu-validate'],
