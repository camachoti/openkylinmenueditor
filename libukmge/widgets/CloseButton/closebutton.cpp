 ...
 * Version Beta 1.0
 *
 * Descrição: Este componente é usado para o botão de fechar da janela. Basta atribuir o pai para gerenciamento automático.
 * Uso:
 *   1. new CloseButton(this); // Usa o botão padrão do tema.
 *   2. new CloseButton(pathPretoSvg, this); // Usa um SVG preto, hover fica branco.
 *   3. new CloseButton(pathPretoSvg, pathBrancoSvg, this) // Usa dois SVGs diferentes.
 */
...
void CloseButton::paintEvent(QPaintEvent *event) {
    // Só desenhar fundo se for hover/pressed (ou seja, cor não transparente)
    if (m_colorBkg.alpha() > 0 && m_colorBkg != palette().color(QPalette::Base)) {
